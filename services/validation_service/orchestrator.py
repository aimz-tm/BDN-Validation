...
from services.document_service.highlights import build_field_highlights
from services.validation_service.logistics import build_validation_logistics
from stub.mock_pipeline import run_mock_validation
...
def run_validation(
    ...
    *,
    ...
    try:
        ...
        from services.ocr_service.classifier import classify as ocr_classify
        ...
        # ── PHASE 2: Extraction ──────────────────────────────────────
        from services.extraction_service.extractor import extract_fields
        ...
        from services.vessel_verification_service.resolver import resolve_vessel_identity
        identity = resolve_vessel_identity(extraction)

        # ── PHASE 5: Barge Verification ───────────────────────────────
        from services.barge_verification_service.resolver import resolve_barge_identity
        ...
        if config.get("pipeline", {}).get("mock_ais", True):
            from stub.mock_ais import get_mock_ais_validation
        ...
        from services.fraud_service.detector import detect_fraud
        fraud = detect_fraud(
        ...
        # ── PHASE 8: Scoring ──────────────────────────────────────────
        from services.scoring_service.scorer import compute_score
        score = compute_score(
        ...
        # ── PHASE 9: Report ───────────────────────────────────────────
        from services.report_service.generator import generate_report
