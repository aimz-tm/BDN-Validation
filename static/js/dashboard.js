...
function setupUpload() {
  ...
  // Direct change listener — user clicks the native file input button
...
function setFile(file) {
  ...
  if (hint) hint.textContent = file.name + " \u2014 click Browse to change";
  const browseBtn = $("#browse-btn");
  ...
  showDocumentStage();
  $("#scan-status").textContent = "Document loaded — click Validate to scan and verify";
...
async function validateUpload() {
  ...
  // Show stepper on results page overlay
...
function updateComplianceChecklist(data) {
  ...
  const density = parseFloat(ext.density);
  const densityStatusEl = $("#comp-density-status");
  const densityValEl = $("#comp-density-val");
  if (densityStatusEl && densityValEl) {
    if (isNaN(density)) {
      densityValEl.textContent = "(no data)";
      setStatusBadge(densityStatusEl, "SUSPICIOUS");
    } else {
      densityValEl.textContent = `(${density.toFixed(3)} kg/m³)`;
      if (density >= 0.820 && density <= 1.010) {
        setStatusBadge(densityStatusEl, "VALID");
      } else {
        setStatusBadge(densityStatusEl, "HIGH_RISK");
      }
    }
  }
  ...
  const flashpoint = parseFloat(ext.flashpoint);
  const flashpointStatusEl = $("#comp-flashpoint-status");
  const flashpointValEl = $("#comp-flashpoint-val");
  if (flashpointStatusEl && flashpointValEl) {
    if (isNaN(flashpoint)) {
      flashpointValEl.textContent = "(no data)";
      setStatusBadge(flashpointStatusEl, "SUSPICIOUS");
    } else {
      flashpointValEl.textContent = `(${flashpoint.toFixed(1)}°C)`;
      if (flashpoint >= 60.0) {
        setStatusBadge(flashpointStatusEl, "VALID");
      } else {
        setStatusBadge(flashpointStatusEl, "HIGH_RISK");
      }
    }
  }
...
function renderVerdict(data) {
  ...
  if (headerVesselName) {
    headerVesselName.textContent = ext.vessel_name || data.vessel_name || "—";
  }
  if (headerImo) {
    headerImo.textContent = ext.imo ? `IMO ${ext.imo}` : (data.imo ? `IMO ${data.imo}` : "IMO —");
  }
  if (headerVerdict) {
  ...
  $("#map-container").innerHTML = ev.map_html || "<p class='status-msg'>No map data</p>";
...
async function loadTransactions() {
  ...
  const url = filter ? `/transactions?classification=${encodeURIComponent(filter)}` : "/transactions";
...
async function loadReviewQueue() {
  ...
  if (!rows.length) {
    tbody.innerHTML = "<tr><td colspan='5'>No items in review queue</td></tr>";
    return;
  }
  tbody.innerHTML = rows
    .map(
  ...
  navItems.forEach(item => {
    item.addEventListener("click", e => {
      ...
      if (viewId === "view-landing") {
        ...
        const browseBtn = $("#browse-btn");
        if (browseBtn) browseBtn.textContent = "Browse Files";
...
function init() {
  try {
    ...
    // Update fields
    const uf = $("#update-fields-btn");
    if (uf) uf.addEventListener("click", handleUpdateFields);
    // History filter
    const filterSel = $("#filter-verdict");
    if (filterSel) filterSel.addEventListener("change", loadTransactions);
    checkHealth();
    loadTransactions();
    loadConfig();
  } catch (err) {
